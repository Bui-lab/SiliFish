...
namespace SiliFish.DynamicUnits
{
    public class HodgkinHuxley : DynamicUnit
    {
        ...
        public HodgkinHuxley(double Vr, double Vmax)
        {
            this.Vr = Vr;
            this.Vmax = Vmax;
            V = Vr;
       ...
       /* public override (Dictionary<string, double> MinValues, Dictionary<string, double> MaxValues) GetSuggestedMinMaxValues()
        ...
        private double IK { get { return g_K * Math.Pow(n, 4) * (V - E_K); } }
        private double INa { get { return g_Na * Math.Pow(m, 3) * h * (V - E_Na); } }
        private double IL { get { return g_L * (V - E_L); } }

        private double alpha_n { get { return 0.01 * (10 - V) / (Math.Exp((10 - V) / 10) - 1); } }
        private double beta_n { get { return 0.125 * Math.Exp(- V / 80); } }
        private double alpha_m { get { return 0.1 * (25 - V) / (Math.Exp((25 - V) / 10) - 1); } }
        private double beta_m { get { return 4 * Math.Exp(-V / 18); } }
        private double alpha_h { get { return 0.07 * Math.Exp(-V / 20); } }
        private double beta_h { get { return 1 / (Math.Exp((10 - V) / 10) + 1); } }
