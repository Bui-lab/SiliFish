namespace Namespace {
    ...
    using animation = matplotlib.animation;
    
    using mode = statistics.mode;
    
    using find_peaks = scipy.signal.find_peaks;
...
";
        ...
        //     
        public static object autocorr(object x) {
            var result = np.correlate(x, x, mode: "full");
            result = result[Convert.ToInt32(result.size / 2)];
        ...
        //     :return: 1-D numpy array
        //     
        public static object Xcorr(object x, object y) {
            var result = np.correlate(x, y, mode: "full");
            result = result[Convert.ToInt32(result.size / 2)];
            return result / np.max(result);
        }
        ...
        //     :return: 1-D numpy array
        //     
        public static object Xcorr_LR(object x, object y) {
            var result = np.correlate(x, y, mode: "full");
            return result / np.max(result);
        }
        ...
        //     :return: 1-D numpy array
        //     
        public static object norm_Xcorr(object x, object y) {
            // Normalised_CrossCorr = 1/N * sum{ [x(n) - mean(x)]* [y(n) - mean(y)] }/ (sqrt(var(x)*var(y))
            var x_adjusted = x - np.mean(x);
            var y_adjusted = y - np.mean(y);
            var result = np.correlate(x_adjusted, y_adjusted, mode: "full") / (x.Count * np.sqrt(np.var(x) * np.var(y)));
            return result;
        }
        ...
        public static object angles_(
            ...
            var ani = animation.FuncAnimation(fig, animate, np.arange(1, Time.Count, 100), interval: 10, blit: false, init_func: init);
        ...
        //     
        public static object recalc_muscle_ouptut(object VLMN, object VRMN, object Time, object dt,            object nMN, object nMuscle, object R, object C, object weight_MN_Muscle) 
        {
            var nmax = Time.Count;
