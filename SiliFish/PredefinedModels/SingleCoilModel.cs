...
namespace SiliFish.PredefinedModels
{
    public class SingleCoilModel : PredefinedModel
    {
        ...
        protected override void InitNeurons()
        {
            ...
            for (int i = 0; i < nIC; i++)
            {
                L_IC.AddCell(new Neuron("IC", seq: i + 1, ic_dyn, sigma_dyn, new Coordinate(x: 1.0 * GetXNoise(), -1), cv: cv, stim: stim));
                R_IC.AddCell(new Neuron("IC", seq: i + 1, ic_dyn, sigma_dyn, new Coordinate(x: 1.0 * GetXNoise(), 1), cv: cv));
            }
